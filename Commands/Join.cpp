...
void    Server::Join(int index, int id)
{
    ...
    for (size_t i = 0; i < channels.size(); i++)
    {
        if (channels[i].getChannelName() == commands[index + 1])
        {
            ...
            {
                channels[i].getClients()[j].print(":" + clients[id].getUserName() + "!" + clients[id].getUserName() + "@localhost" + " JOIN " + commands[index + 1] + "\r\n");
                std::cout << "clients[id].print: " << channels[i].getClients()[j].getUserName() << std::endl;
            }
            for (size_t k = 0; k < channels[i].getClients().size(); k++)
            {
                ...
                std::cout << "channels[i].print: " << channels[i].getClients()[k].getNickName() << std::endl;
            }
        }
    }
